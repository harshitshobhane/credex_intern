      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 relative">
          ...
          {steps.map((step, index) => (
            <div 
              key={index} 
              className="flex flex-col items-center text-center p-8 rounded-xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-800 shadow-lg hover:shadow-xl transition-all duration-300 animate-fade-in transform hover:-translate-y-1 relative z-10"
            ...
            >
              ...
              {index < steps.length - 1 && (
                <div className="hidden md:block absolute top-1/2 -right-5 w-10 h-10 transform rotate-45 border-t-2 border-r-2 border-softsell-blue/30"></div>
              )}
            </div>
          ))}
        </div>
