...
const TestimonialsSection: React.FC = () => {
  ...
  return (
    <section id="testimonials" className="section bg-gradient-to-b from-white to-blue-50 dark:from-gray-900 dark:to-blue-950/50 py-20">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          {testimonials.map((testimonial, index) => (
            <div 
              key={index} 
              className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm p-8 rounded-xl border border-gray-100 dark:border-gray-700 shadow-lg hover:shadow-xl transition-all duration-300 animate-fade-in transform hover:-translate-y-2"
              style={{ animationDelay: `${index * 200}ms` }}
            >
              <div className="flex items-center mb-4">
                {[...Array(testimonial.stars)].map((_, i) => (
                  <Star key={i} className="h-5 w-5 text-softsell-accent fill-softsell-accent animate-pulse-subtle" style={{ animationDelay: `${i * 100}ms` }} />
                ))}
              </div>
